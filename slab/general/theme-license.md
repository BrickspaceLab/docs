# Theme license

Slab is sold directly from the [Brickspace Lab website](https://brickspacelab.com) with an annual subscription. After purchasing, you receive a license key that can be assigned to one Shopify store. When you install the theme, it will prompt you to enter your license key — this activates the license and assigns it to that store.

{% hint style="warning" %}
If your license expires, you can continue using the theme but will no longer receive new updates or support until the license is renewed.
{% endhint %}

<details>

<summary>How do I obtain a license</summary>

Purchase Slab directly from [brickspacelab.com](https://brickspacelab.com). After purchase, you will receive a license key via email and through your customer portal.

</details>

<details>

<summary>How do I install the theme</summary>

After purchasing, you can download Slab as a ZIP file or access the source code directly via GitHub. During installation, the theme will prompt you to enter your license key, which activates and assigns the license to that store.

</details>

<details>

<summary>What happens when my license expires</summary>

You can continue using the version of the theme you have installed. However, access to new updates and support is suspended until the license is renewed.

</details>

<details>

<summary>Can I use one license for multiple stores</summary>

No. Each license key can only be assigned to one Shopify store. You will need to purchase a separate license for each additional store.

</details>

<details>

<summary>Can I transfer my license to a different store</summary>

License transfers between stores are only available in rare cases and are not guaranteed. Contact us to discuss your situation.

</details>
